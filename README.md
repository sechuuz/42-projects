# 42 Network - Validated Projects (sechuuz)

## Project Roadmap

| Circle | Project | Area | Language | Status | Grade |
| :---: | :--- | :--- | :---: | :---: | :---: |
| **≈** | [C Piscine](./c-piscine) | Shell & C Fundametals, Logic | ``c`` | ✅ | LVL 7.67 |
| **00** | [Libft](./rank00/libft) | Static Libraries, `libc` | ``c`` | ✅ | 125/100 |
| **01** | [ft_printf](./rank01/ft_printf) | Variadic Functions | ``c`` | ✅ | 125/100 |
| **01** | [get_next_line](./rank01/get_next_line) | `fd`, Static Variables | ``c`` | ✅ | 125/100 |
| **01** | [born2beroot](./rank01/born2beroot) | Network & SysAdmin, Virtualization | ``shell`` | ✅ | 125/100 |

## Technical Skills

### Languages & Logic
<p align="left">
  <img src="https://img.shields.io/badge/c-%2300599C.svg?style=for-the-badge&logo=c&logoColor=white" />
  <img src="https://img.shields.io/badge/shell_script-%23121011.svg?style=for-the-badge&logo=gnu-bash&logoColor=white" />
</p>

### System Admin & Infrastructure
<p align="left">
  <img src="https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black" />
  <img src="https://img.shields.io/badge/Debian-D70A53?style=for-the-badge&logo=debian&logoColor=white" />
  <img src="https://img.shields.io/badge/VirtualBox-214294?style=for-the-badge&logo=VirtualBox&logoColor=white" />
</p>

### Tools & Dev Environment
<p align="left">
  <img src="https://img.shields.io/badge/git-%23F05033.svg?style=for-the-badge&logo=git&logoColor=white" />
  <img src="https://img.shields.io/badge/Makefile-00599C?style=for-the-badge&logo=gnu&logoColor=white" />
  <img src="https://img.shields.io/badge/Vim-%2311AB00.svg?style=for-the-badge&logo=vim&logoColor=white" />
</p>